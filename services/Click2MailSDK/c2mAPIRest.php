...
class c2mAPIRest
{
	function initc2mAPIRest($un,$pw,$live)
	{
		$this->username = $un;
		$this->password = $pw;
	...
	public function document_create($file, $filename, $documentClass)
	{
	$docName = $filename;
	...
	public function job_create($request)
	{
		$ar = array(
			...
			'rtnName' => $request['rtnName'],
			'rtnaddress1' => $request['rtnaddress1'],
			'rtnCity' => $request['rtnCity'],
			'rtnState' => $request['rtnState'],
			'rtnZip' => $request['rtnZip'],
		);
	...
	public function runAll($job_data, $file, $filename, $xml)
	{
		// echo "Document Uploading\n\n";
		$this->document_create($file, $filename, $job_data['documentClass']);	
		// echo "AddressList Uploading\n\n";
		$this->addressList_create($xml);
		// echo "Job Create\n\n";
		$this->job_create($job_data);
		// echo "Job Submit\n\n";	
		$this->job_Submit();
	...
	{
		$address = new address();
		$address->First_name = $request['recipient']['firstName'];
		$address->Last_name = $request['recipient']['lastName'];
		$address->organization = null;
		$address->Address1 = $request['recipient']['address'];
		$address->Address2 = null;
		$address->City = $request['recipient']['city'];
		$address->State = $request['recipient']['state'];
		$address->Zip = $request['recipient']['zip'];
		$address->Country_nonDASHUS = $request['recipient']['country'];
